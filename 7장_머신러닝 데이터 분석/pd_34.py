...
print('\n')
    ...
    # IPyhton  디스플레이 설정 - 출력할 열의 개수 한도 늘리기
...
print(df.describe())
    ...
    # 3. 속성 선택 ( feature 또는 variable) 선택
...
plt.close()
    ...
    # 4. 훈련/검증 데이터 분할 ( 훈련용=train data, 검증용=test data)
...
X_train, X_test, y_train, y_test = train_test_split(X, y, # 독립변수(X) 와 종속변수(y)
                                                    test_size=0.3, # 검증 30%
                                                    random_state=0) # 랜덤 추출 값
print('train data 개수: ', len(X_train))
print('test data 개수: ', len(X_test))

    # 5. 단순회귀분석 모형 만들기 - sklearn 사용
